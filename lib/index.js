...
const createPrismaMock = (data = {}, datamodel, client = (0, jest_mock_extended_1.mockDeep)(), options = { caseInsensitive: false }) => {
    ...
    const removeMultiFieldIds = (model, data) => {
        ...
        const idFields = model.fields
            .filter((r) => r.name.includes("Id"))
            .map((t) => t.name)
            .slice()
            .reverse() || model.primaryKey?.fields;
        const removeId = (ids) => {
            const id = ids.join("_");
            data = {
                ...data,
    ...
    const Delegate = (prop, model) => {
                        ...
                        if (c.connect) {
                            ...
                            connections.forEach((connect, idx) => {
                                ...
                                if (data[implictTableName]) {
                                    ...
                                    const { originKey, targetKey } = getImplicitKeys(field, model);
                                    ...
                                    if (keyToMatch !== keyToGet) {
                                        ...
                                        if (!matchingRow) {
                                            ...
                                            if (client_1.Prisma.PrismaClientKnownRequestError.length === 2) {
                                                // @ts-ignore
                                                throw new client_1.Prisma.PrismaClientKnownRequestError(message, {
                                                    code,
                                                    clientVersion,
                                                });
                                            }
                                            // @ts-ignore
                                            throw new client_1.Prisma.PrismaClientKnownRequestError(message, code, 
                                            // @ts-ignore
                                            clientVersion);
                                        }
        ...
        return {
            ...
            findFirstOrThrow: (args) => {
                ...
                if (!found) {
                    throw new client_1.Prisma.PrismaClientKnownRequestError("An operation failed because it depends on one or more records that were required but not found. {cause}", "P2025", 
                    // @ts-ignore
                    "1.2.3");
                }
                return found;
